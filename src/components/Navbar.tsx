    ...
    <header className="border-b border-slate-200 bg-white/95 backdrop-blur">
      <nav className="mx-auto w-full max-w-6xl px-4 py-4">
        <div className="flex items-center justify-between">
          <Link href="/" className="text-lg font-semibold tracking-tight text-slate-900" onClick={closeMenu}>
          ...
          <ul className="hidden gap-6 text-sm font-medium text-slate-700 md:flex">
            {navItems.map((item) => (
              <li key={item.href}>
                <Link
                  className={`transition hover:text-brand-600 ${isActive(item.href) ? "text-brand-700" : "text-slate-700"}`}
                  href={item.href}
          ...
          <div className="hidden items-center gap-3 text-sm font-medium md:flex">
            <Link href="/contact" className="rounded-md border border-slate-300 px-3 py-1.5 text-slate-700 transition hover:border-slate-400">
              Contact
            </Link>
            <div className="flex items-center gap-2">
              {socialLinks.map((link) => (
                <a
                  ...
                  title={link.label}
                  className="group relative inline-flex h-8 w-8 items-center justify-center rounded-full border border-slate-300 bg-slate-50 text-slate-700 transition hover:border-brand-300 hover:bg-brand-50 hover:text-brand-700"
                >
                  <link.Icon className="h-4 w-4" />
                  <span className="sr-only">{link.label}</span>
                  <span className="pointer-events-none absolute top-10 left-1/2 z-10 -translate-x-1/2 whitespace-nowrap rounded bg-slate-900 px-2 py-1 text-[10px] font-medium text-white opacity-0 shadow-sm transition-opacity group-hover:opacity-100">
                    {link.label}
                  </span>
                </a>
              ))}
        ...
        <div className={`md:hidden ${isMenuOpen ? "mt-4 block" : "hidden"}`}>
          <ul className="space-y-2 rounded-xl border border-slate-200 bg-white p-3 text-sm font-medium text-slate-700 shadow-sm">
            {navItems.map((item) => (
              <li key={item.href}>
                <Link
                  className={`block rounded-md px-3 py-2 transition hover:bg-slate-100 hover:text-brand-600 ${
                    isActive(item.href) ? "bg-slate-100 text-brand-700" : ""
                  }`}
                  href={item.href}
                  onClick={closeMenu}
                  aria-current={isActive(item.href) ? "page" : undefined}
            ...
            <li className="pt-1">
              <Link
                href="/contact"
                onClick={closeMenu}
                className="block rounded-md border border-slate-300 px-3 py-2 text-center text-slate-700 transition hover:bg-slate-50"
              >
                Contact
              </Link>
            </li>
